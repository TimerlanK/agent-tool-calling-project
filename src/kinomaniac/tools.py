...
def _comma_list(text: str | None) -> list[str]:
    """Split comma/semicolon/newline text into clean unique items."""
...
def _split_candidate_titles(candidate_titles: str) -> list[str]:
    titles = []
    for raw_title in _comma_list(candidate_titles):
...
def _person_matches_movie(movie: dict, person_names: str | None, fields: list[str]) -> bool:
    aliases = [name.lower() for name in _comma_list(person_names)]
    if not aliases:
        return True
...
def _movie_matches_genres(movie: dict, required_genres: str | None) -> bool:
    required = [genre.lower() for genre in _comma_list(required_genres)]
    if not required:
        return True
...
def build_movie_tools(api_key: str):
    ...
    def verify_candidate_movie_titles(
        ...
        allowed_fields = {"Actors", "Director", "Writer"}
        fields = [
            field for field in _comma_list(person_fields)
